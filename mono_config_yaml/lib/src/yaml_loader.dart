...
class YamlConfigLoader implements ConfigLoader {
  ...
  MonoConfig load(String text) {
    ...
    Map<String, List<String>> mapSL(Object? v) {
      ...
      if (v is Map) {
        return {
          for (final e in v.entries) '${e.key}': (e.value is List)
    ...
    final settings = settingsNode is Map || settingsNode is YamlMap
        ? Settings(
            ...
            defaultOrder: (settingsNode['defaultOrder'] ?? 'dependency').toString(),
            shellWindows: (settingsNode['shellWindows'] ?? 'powershell').toString(),
