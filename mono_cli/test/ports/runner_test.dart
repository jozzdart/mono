...
void main() {
  group('Runner', () {
    test('returns 127 and logs when plugin missing', () async {
      ...
      final task = TaskSpec(id: const CommandId('do'), plugin: const PluginId('nope'));
      ...
      expect(logger.byLevel('error').map((e) => e.message).join(' '), contains('No plugin for do'));
    });

    test('returns 127 and logs when plugin does not support command', () async {
      final logger = RecordingLogger();
      final runner = Runner(processRunner: StubProcessRunner(), logger: logger);
      final task = TaskSpec(id: const CommandId('cmd'), plugin: const PluginId('p'));
      ...
      final code = await runner.execute(plan, registry);
      expect(code, 1);
      expect(logger.byLevel('error').map((e) => e.message).join(' '), contains('No plugin for cmd'));
    });
    ...
    test('runs all steps, aggregates non-zero to 1, logs status', () async {
      ...
      final task = TaskSpec(id: const CommandId('run'), plugin: const PluginId('ok'));
      ...
      expect(infoLogs.where((m) => m == 'Done').length, greaterThanOrEqualTo(2));
    ...
    test('passes env from options to plugin execute', () async {
      ...
      final task = TaskSpec(id: const CommandId('id'), plugin: const PluginId('pp'));
