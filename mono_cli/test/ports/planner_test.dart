...
void main() {
  group('DefaultCommandPlanner', () {
    test('creates one step per target preserving order and task', () {
      ...
      final task = TaskSpec(id: CommandId('format'), plugin: const PluginId('format'));
      final targets = [pkg('a'), pkg('b'), pkg('c')];
      final plan = planner.plan(task: task, targets: targets) as SimpleExecutionPlan;
      expect(plan.steps.map((s) => s.package.name.value).toList(), ['a', 'b', 'c']);
