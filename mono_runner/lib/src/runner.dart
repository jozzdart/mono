...
class Runner {
  Runner({required this.processRunner, required this.logger, this.options = const RunnerOptions()});
    ...
    for (final step in plan.steps) {
      results.add(pool.withResource(() async {
        ...
        if (plugin == null || !plugin.supports(step.task.id)) {
          logger.log('No plugin for ${step.task.id} (plugin: ${step.task.plugin?.value ?? 'none'})', scope: step.package.name.value, level: 'error');
          return 127;
        }
        logger.log('Running ${step.task.id.value}', scope: step.package.name.value, level: 'info');
        ...
        if (code != 0) {
          logger.log('Failed with exit code $code', scope: step.package.name.value, level: 'error');
        } else {
