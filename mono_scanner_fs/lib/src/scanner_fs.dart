...
class FileSystemPackageScanner implements PackageScanner {
  ...
  Future<List<MonoPackage>> scan({
    ...
    await for (final entity in dir.list(recursive: true, followLinks: false)) {
      ...
      final matchesInclude = includeGlobsFiles.isEmpty || includeGlobsFiles.any((g) => g.matches(rel));
      if (!matchesInclude) continue;
      final isExcluded = exclude.any((ex) => ex.matches(rel) || ex.matches(p.dirname(rel)));
    ...
    for (final spec in pubspecPaths) {
      ...
      final kind = data.nodes.containsKey('flutter') ? PackageKind.flutter : PackageKind.dart;
      ...
      for (final e in allDeps(spec)) {
        ...
        if (v is YamlMap && v['path'] != null) {
          ...
          if (packages.containsKey(depName)) {
            ...
            final absDep = p.normalize(p.isAbsolute(depPath) ? depPath : p.join(p.dirname(p.join(root, base.path)), depPath));
            final rel = p.relative(absDep, from: root);
            final match = packages.values.firstWhere(
              (pck) => p.normalize(pck.path) == p.normalize(rel),
              orElse: () => MonoPackage(name: PackageName('__none__'), path: '', kind: base.kind),
            );
    ...
    for (final entry in packages.entries) {
      ...
      final withLocal = (spec != null) ? withDeps(entry.value, spec) : entry.value;
